...
## Simbologia militar — quem desenha e quem diz o que existe

Duas coisas diferentes, que é fácil confundir:

- **Quem DESENHA: [milsymbol](https://github.com/spatialillusions/milsymbol)** (integrada desde o protótipo, dependência `npm` desde a Etapa 9a). É o **renderizador** — recebe um código SIDC e devolve o SVG do símbolo pronto para colocar no mapa. Não sabe nada sobre "o que existe": qualquer SIDC bem formado ela desenha.
- **Quem diz O QUE EXISTE: o [Portal de Simbologia Militar do MD/EB](https://simbologia.eb.mil.br/)** (integrado na Etapa 9b). É a fonte de **dados** — 12 categorias, 434 ícones centrais com o rótulo oficial em português (`NomeBR`) e 488 modificadores, baseados no manual **MD33-M-02** e no catálogo **MD33-C-01**. É o que alimenta os seletores do formulário de marcação e o que substituiu as tabelas reduzidas escritas à mão (`NATUREZA`, `DIMENSAO`).

  Capturado uma vez e versionado: extrato normalizado em `data/simbologia-eb/` (procedência com URL, SHA-256 e data em `data/simbologia-eb/PROCEDENCIA.md`), módulo gerado em `frontend/simbolos-catalogo.js` por `scripts/gerar-catalogo-simbologia.mjs`. **Em campo o app não fala com o portal** — ver a decisão do item 2 da Etapa 9b.

**Sobre o [stanag-app6](https://github.com/spatialillusions/stanag-app6)** (do mesmo autor da milsymbol), que este documento indicava até a Etapa 9a como a fonte de tabelas a adotar: **a hipótese foi verificada e descartada** durante a discussão da Etapa 9b. Ele não é um catálogo de símbolos utilizável como fonte de rótulos — e, ainda que fosse, seria a taxonomia da OTAN em inglês, não a brasileira em português. Não é dependência do projeto e não deve ser adicionado.
...
frontend/       app web (Leaflet + milsymbol + stanag-app6). Login/cadastro/roteamento por papel prontos
                ...
                Etapa 9b: simbolos-catalogo.js é o catálogo oficial do MD/EB GERADO (não editar à
                mão — ver scripts/gerar-catalogo-simbologia.mjs) e é de onde simbolos.js passou a
                tirar DIMENSAO/NATUREZA; coordenadas.js é a conversão pura lat/lon -> UTM/decimal/
